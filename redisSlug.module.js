...
function slug(string, opts, cb) {
    ...
    for (var char, i = 0, l = string.length; i < l; i++) { char = string[i];
        if (!lengths.some(function (len) {
            ...
            } else if (char.match(possibleUnknown)) {
...
function populate(cb) {
  var
    ...
    unicodeAddMulti = client.multi();
  
  _symbols = _.mapValues(_symbols,function(unicodeObj) {
    var
      unicodeName = unicodeObj.name.toLowerCase();
      
    //remove any words from the removelist
    for(var j = 0, rl = removelist.length; j < rl; j++) {
      unicodeName = unicodeName.replace(removelist[j], '');
    }
    
    //remove any unallowed characters
    unicodeName = unicodeName.replace(/[^\w\s\-\.\_~]/g, '');
    return unicodeName;
  });

  _.forOwn(_symbols, function(unicodeName,field) {
    unicodeAddMulti.hset('unicode',String.fromCharCode(field),unicodeName);
  });
  
  unicodeAddMulti.exec(cb);
}
